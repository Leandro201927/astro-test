...
export async function getAllMedia(locals: App.Locals) {
  ...
  const list = await kvHelper.list(locals, { prefix: "media:" });
  const items: any[] = [];
  for (const k of list.keys || []) {
    ...
    const value = await kvHelper.get<any>(locals, kvKey, { type: 'json' });
